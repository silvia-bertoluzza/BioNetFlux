...
### 3.2 Things that always need my approval
...
### 3.5 External libraries require approval
...
- **`TimeStepResult` duplication**: This dataclass currently exists in both `time_step_result.py` and `time_stepper.py`. This is a known issue to be resolved. The dataclass in `time_step_result.py` is deprecated, use the one in `time_stepper.py`.
