...
class StaticCondensationOOCUpwind(StaticCondensationBase):
    ...
    def static_condensation(self, local_trace, local_source=None, **kwargs):
        ...
        if prev_local_solution is not None:
            ...
            phi0 = u4_prev[0]  # Assuming phi is u4
            phi1 = u4_prev[1]  # Assuming phi is u4
            chi_frozen = np.array([[self.chi_func(phi0)], [self.chi_func(phi1)]])  # shape (2,1) so chi_frozen.T @ M @ u1 is valid
            grad_phi_frozen = (phi1 - phi0) / h  # Example of freezing grad_phi
        else:
        ...
        # print(f"DEBUG: chi_frozen: {chi_frozen}, grad_phi_frozen: {grad_phi_frozen}")
        ...
        j = hB4 @ hU + grad_phi_frozen * chi_frozen.T @ Q @ U
