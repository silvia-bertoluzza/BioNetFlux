...
> **STATUS: PROPOSAL ONLY — NOT YET IMPLEMENTED.** This document describes aspirational
> improvements to BioNetFlux's user experience. None of the proposed `user_api/`, `demos/`,
> or `templates/` directories have been created. The current interface uses `quick_setup()`,
> `TimeStepper`, and problem modules directly (see `examples/` for working usage patterns).
