...
This section provides an exact reference for the Problem class (\texttt{bionetflux.core.problem.Problem}) based on detailed analysis of the actual implementation. The Problem class serves as the base container for single arc mathematical problem specification in BioNetFlux.
...
\begin{itemize}
    ...
    \item \texttt{domain\_start}: Arc start coordinate (default: 0.0)
    \item \texttt{domain\_length}: Arc length (default: 1.0)
...
\texttt{extrema} & \texttt{List[Tuple]} & Arc endpoints coordinates: \texttt{[(domain\_start, 0.0), (domain\_start + domain\_length, 0.0)]} \\
\hline

\texttt{neumann\_data} & \texttt{np.ndarray} & Boundary data array initialized as {\color{red} \texttt{np.zeros(4)} }\\
\hline
...
\paragraph{set\_boundary\_flux()}\leavevmode
...
\paragraph{get\_parameter()}\leavevmode
...
\paragraph{set\_parameter()}\leavevmode
...
\paragraph{set\_parameters()}\leavevmode
...
\paragraph{set\_extrema()}\leavevmode
...
\paragraph{get\_extrema()}\leavevmode
...
\paragraph{set\_function()}\leavevmode
...
\begin{todobox}
	\begin{itemize}[leftmargin=1.5em]
		\item Clarify the difference bewteen {\tt u\_names} and {\tt unknown\_names}. Possibly eliminate one of the two
		\item Check {\tt neumann\_data}, it has hard coded dimension 4. Probably not used.
		\item  Have problem point to a geometry, instead of having its own geometry handling attributes and methods
		\item Let chemotaxis functions be defined via the {\tt set\_function} method rather than a dedicated method
		\item Are {\tt \{left,right\}\_flux} used also for Dirichlet data? Consider changin name to {\tt \{left,right\}\_boundary\_data}
		\item Add handling of parameters using names to identify the different parameters
		\item In derived problems initial condition uses a different name than {\tt u0}. This can cause problem, find a way to ensure that everything runs as it should
		\item Add documentation for the test suite methods: {\tt validate\_problem}, {\tt test\_functions}, {\tt create\_test\_problems} and {\tt run\_self\_test}
	\end{itemize}
\end{todobox}
