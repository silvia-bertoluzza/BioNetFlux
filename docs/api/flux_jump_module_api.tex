...
\section{Flux Jump Module API Reference}
\label{sec:flux_jump_module_api}

This section provides an exact reference for the flux jump module (\texttt{bionetflux.core.flux\_jump}) based on detailed analysis of the actual implementation. The module computes local flux balance contributions for HDG methods.
...
\begin{structurebox}
	The data is of two types: trace data and bulk data. Bulk data is handled by the {\tt BulkData} and {\tt BulkDataManager} classes, and it is organized as {\tt 2 neq} $\times$ {\tt n\_elements} array. In each column, the 2 coefficients of  physical function of index {\tt i\_eq} (with $0 \leq {\tt i\_eq} \leq {\tt n\_eq}-1$) in the corresponding element are the $2 {\tt  i\_eq}$ and $2 {\tt i\_eq} + 1$. 
	
	Trace data are stacked in a single array of length {\tt n\_eq} $\times$ {\tt n\_nodes}. Here the value of the physical function of index {\tt i\_eq} in the node {\tt i\_node} is {\tt n\_nodes} $\times$ {\tt i\_eq} $+$ {\tt i\_node}.
\end{structurebox}

For element k, the function constructs local indices as:
...
\begin{todobox}
	Add a section on data structures, somewhere.
\end{todobox}
...
\begin{itemize}
    ...
    \item \textbf{Static Condensation Integration}: Compatible with all admissible static condensation implementations
...
\begin{todobox}
	\begin{itemize}
		\item Clean up the implmenentation by removing the dummy placeholders for problem and discretization
	\end{itemize}
\end{todobox}
