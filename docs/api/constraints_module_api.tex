...
This section provides an exact reference for the constraints module (\texttt{bionetflux.core.constraints}). The module handles boundary conditions and junction constraints using Lagrange multipliers for HDG methods.
...
\begin{itemize}
    ...
    \item Constraint residual computation for  solvers
\end{itemize}
...
\begin{itemize}
    \item \textbf{Boundary Conditions}:  (DIRICHLET, NEUMANN, ROBIN) Single domain constraints
    \item \textbf{Junction Conditions}:  (TRACE\_CONTINUITY, KEDEM\_KATCHALSKY)
Multi-domain constraints \end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{equation\_index}: Index of the equation (0, 1, ...) the constraint applies to
    \item \texttt{domains}: List of indices of the domains the constraint applies to (length 1 for boundary, 2 for junction)
    \item \texttt{positions}: Position (parametric) coordinates in each domain where the constraint is applied
    \item \texttt{parameters}: Constraint coefficient (e.g., Robin coefficients) (optional)
...
\textbf{Returns:} \texttt{float} - Value of constraint data at given time
...
In each one of the domain involved, the constraint is imposed at the discretization node closest to the position. The corresponding node indices are stored in the {\tt \_node\_mappings} attribute of the {\tt ConstraintManager} object. More precisely, depending on whether the index {\tt icx} constraint is a boundary or a junction constraint,  {\tt contraints.\_node\_mappings[icx]} contains a length $1$ or $2$ list containing the index of the node(s) where the constraint is applied on the domain(s) {\tt idx} (for boundary constraint) or {\tt idx1, idx2} (for junction constraint).
...
% Usage example
\begin{deprecatedbox}
{\bf This method is deprecated and will be removed in future versions.}

This method assumes {\tt neq}=2 and should ONLY be used for Keller-Segel problems.
For general problems, use the {\tt domain\_data\_list parameter} in {\tt  compute\_constraint\_residuals}
and access {\tt len(domain\_data\_list[domain\_idx].nodes)} directly.
\end{deprecatedbox}
...
def newton_solve_with_constraints(constraint_manager, initial_solution, initial_multipliers):
    ...
    for iteration in range(max_iterations):
        ...
        if residual_norm < tolerance:
            print("\checkmark Newton solver converged")
            break
        ...
        try:
            ...
            print("\times Newton solver failed: singular Jacobian")
            break
...
\begin{todobox}
		\begin{itemize}[leftmargin=1.5em]
			\item Add some external validation of the correct inclusions of the constraint position in the range of the domain parameter
			\item Add the possibility of prescribing the jump of the solution at a point
			
			\item Add the possibility of handling a branching junction with more than 2 domains (will affect {\tt n\_multipliers} both in {\tt Constraint} and in {\tt ConstraintManager})
			
			\item {\tt n\_multipliers} is a method of both {\tt Constraint} (where it returns 2) and of {\tt ConstraintManager} (where it returns the global number of multipliers). Evaluate changing the name of the first one into {\tt num\_local\_multipliers} to avoid errors
			
			\item Check the Newton integration example (see commented section in latex file) - Missing global assembly integration
			\item The function {\tt add\_constraints\_from\_geometry} should belong to this module and have the type of constraints as input
			\end{itemize}
\end{todobox}
