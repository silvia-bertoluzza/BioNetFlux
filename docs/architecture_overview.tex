...
\begin{center}
\begin{tikzpicture}[
    pkg/.style={draw, rounded corners, fill=blue!8, minimum width=3.4cm, minimum height=0.8cm, font=\ttfamily\small},
...
% Sub-packages row
\node[pkg] (core)    at (-7, -2) {core/};
\node[pkg] (geom)    at (-3.5,-2) {geometry/};
\node[pkg] (probs)   at (0,  -2) {problems/};
\node[pkg] (ti)      at (3.5,-2) {time\_integration/};
\node[pkg] (utils)   at (7,  -2) {utils/};
\node[pkg] (vis)     at (10.5,-2) {visualization/};

% analysis
\node[pkg, fill=orange!10] (anal) at (-10.5,-2) {analysis/};
