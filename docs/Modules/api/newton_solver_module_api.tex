...
\texttt{final\_residual\_norm} & \texttt{float} & $\ell$2 norm of residual at final iteration \\
\hline

\texttt{residual\_history} & \texttt{List[float]} & Residual norm at each Newton iteration \\
\hline

\texttt{step\_norms} & \texttt{List[float]} & $\ell$2 norm of Newton step at each iteration \\
\hline
...
for iteration in range(max_iter):
    ...
    try:
        current_res, current_jac = global_assembler.assemble_residual_and_jacobian(
...
The Newton solver module integrates directly with BioNetFlux components through the solve method interface, which relies on the {\tt compute\_residual\_and\_jacobian} method of the module {\tt GlobalAssembler} to compute residual and jacobian :
...
\begin{itemize}
    \item \textbf{Global Solution Vector}: Newton operates on global DOF vector including traces and multipliers, the structure of which (particularly in relation with the correspondence with the different arcs and equations) is handled by the global assembler
    \item \textbf{Forcing Terms}: Forcing terms are not vectors with the same structure as the global solution vectors, but bulk data type dual objects. These are computed by BulkDataManager and static condensation
...
\begin{todobox}
	Design a more general solver module that can call the newton solver but also other non linear solvers, particularly Picard method.
\end{todobox}
