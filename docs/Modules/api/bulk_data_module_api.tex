...
This section provides reference for the BulkData class  (\texttt{bionetflux.core.bulk\_data}). The BulkData class manages the coefficients of bulk objects for HDG methods. A bulk object contains coefficients for discrete objects ``living'' in the elements. This class handles both element solutions, which are marked as {\tt not dual} (primal), by providing the coefficients with respect to the chosen basis for the element (the choice of the basis is set in the {\tt elementary\_matrices} module), and right hand side object, marked as {\tt dual}. For dual objects the coefficients contained in the BulkData field {\tt data} are the integrals of the considerefd function with the elements of the chosen basis. 
...
\begin{itemize}
    ...
    \item \texttt{dual}: Boolean flag for distinguishing between primal (solutions, {\tt dual} = False) and dual (right hand side, {\tt dual} = True) bulk objects (default: False)
\end{itemize}
...
\texttt{dual} & \texttt{bool} & Flag for dual  mode \\
\hline
...
\subsection{Data Setting Method}
\label{subsec:set_data_method}
{\tt set\_data} populates the {\tt data} field of a BulkData object, in such a way that the {\tt j}-th column of {\tt data} contains the coefficients of {\tt neq} functions on the {\tt j}-th element.
...
Depending of whether a BulkData object is tagged as {\tt dual} or not, and on the format of the input, different methods are called to evaluate the {\tt data} field entries 
...
if bulk_data.test():
    print("\checkmark BulkData instance is valid and functional")
else:
    print("\times BulkData instance has issues")
\end{lstlisting}
...
\begin{todobox}
\begin{itemize}
	\item Evaluate better name for the {\tt BulkData} class ({\tt LocalBulkData}?)
	\item Evaluate merging module with the {\tt lean\_bulk\_data\_manager} module
	\item Check \& fix for use with orthonormal basis functions
	\item Correct the {\tt compute\_mass} method to handle each unknown independently
	\item The {\tt BulkData} initialize from functions uses functions with two arguments (s and t). It would be cleaner if it used only one, and the setting of time was done from outside
\end{itemize}
\end{todobox}
