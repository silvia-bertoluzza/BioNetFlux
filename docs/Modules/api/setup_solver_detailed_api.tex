...
This section provides an exact reference for the setup solver module (\texttt{setup\_solver.py}) based on detailed analysis of the actual implementation. The module provides lean solver orchestration for problems on networks, minimizing data redundancy while providing clean interfaces for different problem types.
...
\textbf{Parameters:}
\begin{itemize}
    \item \texttt{problem\_module}: String path to problem module containing \texttt{create\_global\_framework} (default: "bionetflux.problems.KS\_traveling\_wave")
    \item \texttt{config\_file}: String path to the TOML configuration files containing the values of the different  parameters needed by the problem module
    \item \texttt{geometry}: {\tt DomainGeometry} object describing the geometry on which the problem has to be solved
\end{itemize}

\textbf{Side Effects:} 
\begin{itemize}
    \item Sets \texttt{self.problem\_module} to specified module path
    \item Sets {\tt self.config\_file} to specified configuration file
    \item Sets {\tt input\_geometry} to specified {\tt DomainGeometry} object
    \item Initializes \texttt{self.\_initialized} to False
    \item Sets all other framework object attributes to None
    \item Sets all cached component attributes to None
\end{itemize}

\textbf{Usage:}
\begin{lstlisting}[language=Python, caption=Constructor Usage Examples]
# Default problem module (Keller-Segel with default parameters and geometry)
setup = SolverSetup()

# Custom problem module with default geometry and configuration
setup = SolverSetup("bionetflux.problems.ooc_problem")

# OrganOnChip problem with user defined parameters and geometry
setup = SolverSetup("bionetflux.problems.ooc_problem",config\_file="config.ooc_parameters",geometry = geometry)
\end{lstlisting}

\subsubsection{Core Attributes}

\begin{longtable}{|p{4.1cm}|p{5.8cm}|p{4cm}|}
\hline
...
\texttt{config\_file} & \texttt{Optional[str]} & String path to problem TOML configuration file \\
\hline

\texttt{input\_geometry} & \texttt{Optional[DomainGeometry]} & Geometry provided as input \\
\hline
...
\texttt{geometry} & \texttt{Optional[DomainGeometry]} & Geometry computed from problems extrema \\
\hline

\texttt{constraints} & \texttt{Optional[ConstraintManager]} & \texttt{constraint\_manager} alias for backward compatibility \\
\hline
...
\paragraph{initialize()}\leavevmode
...
\begin{enumerate}
    ...
    \item Call function with geometry and config\_file parameters to get: problems, global\_discretization, constraint\_manager, problem\_name
...
\paragraph{\_ensure\_initialized()}\leavevmode
...
\paragraph{elementary\_matrices}\leavevmode
...
\paragraph{static\_condensations}\leavevmode
...
\paragraph{global\_assembler}\leavevmode
...
\paragraph{bulk\_data\_manager}\leavevmode
...
\paragraph{get\_problem\_info()}\leavevmode
...
\paragraph{create\_initial\_conditions()}\leavevmode
...
\paragraph{create\_global\_solution\_vector()}\leavevmode
...
\paragraph{extract\_domain\_solutions()}\leavevmode
...
\paragraph{compute\_geometry\_from\_problems()}\leavevmode
\begin{lstlisting}[language=Python, caption=Extract Domain Solutions Method]
...
\textbf{Parameters:}
\begin{itemize}
	\item \texttt{geometry\_name}: String with the name of the geometry
\end{itemize}

\textbf{Returns:} \texttt{DomainGeometry} - geometry where the problems are set
...
# Add domain to geometry
	geometry.add_domain(extrema_start=extrema_start,
...
\textbf{Purpose:} Retrive geometry when this has not be stores in the setup
\textbf{Usage:}
\begin{lstlisting}[language=Python, caption=Domain Solution Extraction Usage]
geometry = setup.compute_geometry_from_problems("grid_geometry")
\end{lstlisting}
...
\paragraph{validate\_setup()}\leavevmode
...
\paragraph{create\_solver\_setup()}\leavevmode
...
\textbf{Parameters:}
\begin{itemize}
    \item \texttt{problem\_module}: String path to problem module (default: "bionetflux.problems.ks\_problem")
        \item \texttt{config\_file}: String path to the TOML configuration files containing the values of the different  parameters needed by the problem module
    \item \texttt{geometry}: {\tt DomainGeometry} object describing the geometry on which the problem has to be solved
\end{itemize}
...
setup = create_solver_setup("bionetflux.problems.ooc_test_problem")
...
\paragraph{quick\_setup()}\leavevmode
\begin{lstlisting}[language=Python, caption=Quick Setup Function]
def quick_setup(problem_module: str = "bionetflux.problems.ks_problem", 
               validate: bool = True,
               config_file: Optional[str] = None,
               geometry: Optional['DomainGeometry'] = None) -> SolverSetup
\end{lstlisting}
...
\textbf{Parameters:}
\begin{itemize}
    \item \texttt{problem\_module}: String path to problem module (default: "bionetflux.problems.KS\_traveling\_wave")
...
setup = quick_setup("bionetflux.problems.ooc_problem", validate=True)

# Quick setup without validation (faster)
setup = quick_setup("bionetflux.problems.ks_problem", validate=False)
\end{lstlisting}
...
setup = quick_setup(
			...
			validate=True,
			config_file="config.ks_parameters",  # Pass config file
...
for eq_idx in range(plotter.neq):
	plotter.plot_birdview(final_traces,
										equation_idx=eq_idx,
										time=current_time,
...
\texttt{compute\_geometry\_from\_problems} & \texttt{DomainGeometry} & Retrieves geometric data hidden in the {\tt problems} structure \\
\hline
...
\begin{todobox}
	\begin{itemize}
		\item {Choose (possibly new) default problem to be used in {\tt \_\_init\_\_}}
		\item Replace calls to {\tt SolverSetup.constraints} with calls to {\tt SolverSetup.constraint\_manager}
		\item Add handling of missing config\_file
		\item Two attributes for geometry: {\tt geometry} and {\tt input\_geometry}. Merge
		\item Add geometry as a {\tt @property} attribute
		\item Add handling of functions defined in the physical coordinates
	\end{itemize}
\end{todobox}
