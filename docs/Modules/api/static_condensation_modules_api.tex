...
The static condensation module plays a crucial role in the BioNetFlux code. It is the module where the  problem specific HDG discretization happens, and it is therefore the module that needs to be implemented from scratch. This section provides a comprehensive reference for the static condensation module hierarchy based on analysis of the MATLAB reference files and BioNetFlux architecture patterns. The modules implement HDG static condensation for different problem types.
...
\begin{itemize}
    ...
    Two example are presently available in the code
   \begin{itemize}
    \item \textbf{KellerSegelStaticCondensation}: Implementation of full Newton method for 2-equations chemotaxis systems (Keller Segel)
    \item \textbf{StaticCondensationOOC}: Implementation of full Newton method for 4-equation OrganOnChip systems
   \end{itemize}
    \item \textbf{StaticCondensationFactory}: Factory for referring to the appropriate implementation
\end{itemize}
...
{\color{gray}  \texttt{neq}} & {\color{gray}   \texttt{int} }& {\color{gray} Number of equations (extracted from problem)} \\
\hline

{\color{gray}  \texttt{n\_elements}  }& {\color{gray}  \texttt{int}  }& {\color{gray}  Number of elements (extracted from discretization) } \\
\hline

{\color{gray}   \texttt{element\_length}  }&{\color{gray}   \texttt{float}  }& {\color{gray}  Element size: \texttt{h = domain\_length / n\_elements}  }\\
\hline

{\color{gray}   \texttt{dt}  }&{\color{gray}   \texttt{Optional[float]}  }& {\color{gray} Time step size (from discretization if available)} \\
\hline

\end{longtable}

\begin{warningbox}
	The {\tt StaticCondensationBase} class stores references to {\tt self.problem} and {\tt self.discretization} but doesn't actually extract the commonly used parameters like:
	\begin{itemize}
	\item 	{\tt neq} (from {\tt problem.neq})
\item	{\tt n\_elements} (from {\tt discretization.n\_elements})
\item	{\tt element\_length }(from {\tt discretization.element\_length})
\item	{\tt dt} (already extracted: {\tt self.dt} = {\tt global\_disc.dt})
\end{itemize}
	This is a design inconsistency rather than a functional bug. While the code works, it violates the principle of ``extract once, use many times" that's evident in {\tt DomainData}. The base class should probably extract these commonly used parameters to provide a consistent interface for derived classes and align with the framework's data extraction philosophy.
		The inconsistency is more about maintainability and design clarity than immediate functionality.
\end{warningbox}
...
\begin{itemize}
    ...
    \item \texttt{**kwargs}: Additional problem specific parameters
\end{itemize}

\textbf{Returns:} \texttt{Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]} - (bulk\_solution, flux,  flux\_trace, jacobian)

\textbf{Purpose:} Perform local static condensation step


\paragraph{assemble\_forcing\_term}\leavevmode
\begin{lstlisting}[language=Python, caption=Abstract Static Condensation forcing term assembly]
	@abstractmethod
...
\textbf{Parameters:}
\begin{itemize}
	\item \texttt{*args}: Positional arguments
	\item \texttt{**kwargs}: Keyword arguments
\end{itemize}

\textbf{Returns:} \texttt{np.ndarray} - Assembled right-hand side in correct format for static condensation

\textbf{Purpose:} Assemble the right-hand side for the static condensation system.
...
\paragraph{get\_matrices}\leavevmode
\begin{lstlisting}[language=Python, caption=Get Matrices]
...
\textbf{Returns:} \texttt{Dict[np.ndarray]} - Matrices needed for the static condensation
%
%\textbf{Usage:}
...
\begin{warningbox}
	There is an inconsistency regarding the static condensation matrices. The base method has an output (the static condensation matrices are seen as external to the class), in the derived implementations {\tt sc\_matrices} are an attribute to the class. This second behaviour is the one expected by {\tt flux\_jump}.
\end{warningbox}
...
\begin{todobox}
	\begin{itemize}
		\item 	Modify the {\tt static\_condensation} method so that it can return the bulk solution only (no flux jump or jacobian), or the bulk solution + flux jump (no jacobian)
		\item Maybe we need it to be able to return the jacobian only? 
		\item Could modify the factory by including also a check on the solution method: if Newton proceed as now, if Picard, do something else 
		\item Add instructions to the documentation for adding a new type of problems 
		\item Fix inconsistency between implementation and documentation of {\tt build\_matrices} method. It does not work by outputting a dictionary but initializes an {\tt sc\_matrices} attribute of the static condensation class
	\end{itemize}
\end{todobox}
...
def static_condensation(self, 
                       trace_values: np.ndarray, 
                       rhs: np.ndarray, 
                       time: float = 0.0) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]
\end{lstlisting}
...
\begin{enumerate}
    ...
    \item Return bulk solutions, flux, flux trace, and Jacobian
\end{enumerate}
...
bulk_solution, flux, flux_trace, jacobian = ks_sc.static_condensation(
...
\begin{warningbox}
	This example runs but it is not trustworthy. I am not sure that the local to global extraction is performed in the correct way, this needs to be checked. Compare with the actual examples. Also, this example hard-codes the {\tt neq=2} of the KS system. It would be nicer to use an example where {\tt neq} is used in a parametric way
\end{warningbox}
...
\begin{warningbox}
	This example runs but it is not trustworthy. I am not sure that the local to global extraction is performed in the correct way, this needs to be checked. Compare with the actual examples. Also, this example hard-codes the {\tt neq=4} of the OoC system. It would be nicer to use an example where {\tt neq} is used in a parametric way. Look at {\tt flux\_jump} for how to do this
\end{warningbox}
...
\texttt{assemble\_forcing\_term} & \texttt{np.ndarray} & Abstract: assembles right hand side in correct form\\
\hline
...
\texttt{get\_matrices} & \texttt{dict} & Returns matrices for static condensation\\
\hline
...
\texttt{\_compute\_jacobian} & \texttt{np.ndarray} & Computes the Jacobian \\
\hline
...
\texttt{build\_matrices} & \texttt{dict} & Build 4-equation OrganOnChip matrices  \\
\hline

\texttt{static\_condensation} & \texttt{Tuple} & Perform OrganOnChip static condensation  \\
\hline
...
\begin{todobox}
\begin{itemize}
	\item Fix inconsistencies bewteen KS, OoC and Base methods, particularly as computing bulk solution, local jacobian independently from each other are concerned. 
	\item (Related to the previous one) Reduce duplicating implementation
	\item Build new examples of usage that rely on parametric number of equations
\end{itemize}
\end{todobox}
