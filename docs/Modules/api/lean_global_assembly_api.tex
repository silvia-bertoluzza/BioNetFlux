...
This section provides an exact reference for the global assembly module based on  the actual implementation in \texttt{bionetflux.core.lean\_global\_assembly.GlobalAssembler}. This class provides global assembly functionalities needed to construct the global (linear or nonlinear) functionals and operators involved in the solution process, acting on traces from the different arcs, and the lagrangian multipliers.
...
\begin{itemize}
    ...
    \item Degrees of freedom management for multi-domain systems
    \item Comprehensive testing infrastructure
\end{itemize}

\begin{structurebox}
	The solution of the problem at time $t$ consist in a trace vector + two multipliers for each arc and each equation. In total we then have
	\[
	\sum_{{\tt idx}=0}^{{\tt n\_domains}-1} {{\tt neq}[{\tt idx}]} \times (\underbrace{{\tt n\_elements[idx]} + 1}_{\text{\# nodes of arc {\tt idx}}} + 2)
	\]
	unknowns. The individual flux conservation at all nodes of domain {\tt idx} yields \(
	{\tt neq}[{\tt idx}] \times ({\tt n\_elements[idx]} + 1)
	\) equations. The missing \(2 \times {\tt n\_domains}\) equations are  {\tt n\_domains} junction conditions and  {\tt n\_domains} flux balance conditions.
\end{structurebox}
...
\begin{itemize}
    ...
    \item Computes and stores DOF structure and domain offsets
...
\texttt{total\_dofs} & \texttt{int} & Total DOFs: {\tt n\_multipliers} +  \texttt{total\_trace\_dofs}  \\
\hline
...
\begin{notebox}
	Assemble residual and jacobian is a key  method of the whole procedure. Right now it is tuned for Newton's method, but I believe it can be used for a number of methods. It simply calls {\tt domain\_ flux\_jump.} and {\tt compute\_constraint\_residual} from {\tt constraint\_manager} to obtain the global residual and jacobian
	\end{notebox}
...
\textbf{Computation:} Call the {\tt compute\_forcing\_terms} methods from the {\tt BulkData} class
...
\begin{todobox}
	\begin{itemize}
		\item Rename as {\tt global\_assembly} or {\tt global\_data\_assembly}
		
		\item the method {\tt \_add\_constraint\_jacobian\_contribution} refers to the constraint manager to decide what to do, but it belongs to the  global assembly class, maybe this is not the cleanest choice? Evaluate splitting and move the local evaluation to {\tt ConstraintManager}
		
		\item  the method {\tt \_add\_constraint\_jacobian\_contribution} does not require the trace and multiplier values in input, nor does it require the time
		
		\item Check if {\tt compute\_forcing\_terms} in this class is necessary, or if we can resort to {\tt BulkDataManager.compute\_forcing\_terms} instead
		
		\item Same thing for {\tt initialize\_bulk\_data} and for {\tt compute\_mass\_conservation}. Maybe we can choose one of the two option ({\tt GlobalAssembler} or {\tt BulkDataManager}/{\tt BulkData}) and deprecate the other?
		
		
		\item {\tt GlobalAssembler} and {\tt BulkDataManager} are somewhat overlapping, evaluate possibilities (merge?)
